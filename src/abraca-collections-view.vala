...
namespace Abraca {
	...
	public class CollectionsView : Gtk.TreeView {
		...
		public GLib.List<Gtk.Widget> collection_menu_item_when_coll_selected = null;
		public GLib.List<Gtk.Widget> collection_menu_item_when_pllst_selected = null;
		private GLib.List<Gtk.MenuItem> collection_menu_item_when_ns_selected = null;

		private Gtk.MenuItem collection_menu_item_activate;
		...
		private void on_selection_changed_update_menu (Gtk.TreeSelection s)
		{
			...
			bool sens = n==2 && ((CollectionsModel)model).path_is_child_of_type(
					model.get_path(iter), CollectionsModel.CollectionType.Playlist);
			foreach(var i in collection_menu_item_when_pllst_selected)
				i.sensitive = sens;
		}
		...
		public void on_menu_collection_activate() {
			...
			if (get_selection().get_selected(null, out iter)) {
				...
				if (path.get_depth() == 2 && ((CollectionsModel)model).path_is_child_of_type(
							path, CollectionsModel.CollectionType.Playlist)){
					string name;
					model.get(iter, CollectionsModel.Column.Name, out name);
		...
		private void create_context_menu ()
		{
			...
			collection_menu_item_when_pllst_selected.prepend(item);
			collection_menu.append(item);
			collection_menu_item_activate=item;
